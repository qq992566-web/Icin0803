...
# 🚀 全域系統版本號
APP_VERSION = "v2.2.0 (Build 20260803 - Dynamic TTS & Geothermal Energy Guide Link)"
...
# 原始聽力題庫 (15題標準數據庫，完全替換為火熱與地熱主題)
QUIZ_DATA = [
    {"id": 1, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-01.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["namah", "fali", "nanom", "sera"], "correct_text": "namah"},
    {"id": 2, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-02.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["fa'edetay", "sa'etener", "takaraw", "moya"], "correct_text": "fa'edetay"},
    {"id": 3, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-03.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["kasenawan", "kasolin", "lapits", "poy"], "correct_text": "poy"},
    {"id": 4, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-04.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["lamal", "'akong", "kilang", "lotok"], "correct_text": "lamal"},
    {"id": 5, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-05.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["cepo'", "hot spring", "kasenawan", "teked"], "correct_text": "kasenawan"},
    {"id": 6, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-06.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["kinis", "volcano", "fali", "riyar"], "correct_text": "volcano"},
    {"id": 7, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-07.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["tali", " steam", "konga", "poy"], "correct_text": " steam"},
    {"id": 8, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-08.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["power", "heat", "energy", "earth"], "correct_text": "heat"},
    {"id": 9, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-09.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["geothermal", "hydro", "wind", "solar"], "correct_text": "geothermal"},
    {"id": 10, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-10.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["magma", "rock", "stone", "sand"], "correct_text": "magma"},
    {"id": 11, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-11.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["fire", "water", "air", "earth"], "correct_text": "fire"},
    {"id": 12, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-12.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["hot", "cold", "warm", "cool"], "correct_text": "hot"},
    {"id": 13, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-13.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["crust", "mantle", "core", "surface"], "correct_text": "crust"},
    {"id": 14, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-14.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["steam", "lava", "ash", "smoke"], "correct_text": "lava"},
    {"id": 15, "audio_path": "assets/audio/01_listening/listening_words/tengil-a1-15.mp3", "question_text": "聆聽音檔，選出關聯的詞彙：", "options": ["generator", "turbine", "pipe", "well"], "correct_text": "turbine"}
]

# ==========================================
# 🎵 新增功能：南島語系與地熱主題動態發音引擎 (TTS)
# ==========================================
def play_tts(text):
    """
    在上傳實體聲音檔之前，利用印尼語(id)近似發音規則，
    自動萃取題幹中的詞彙並進行動態發音。
    """
    # 1. 嘗試抓取「」內的詞彙 (針對選擇題)
    ...
    if match:
        ...
        # 2. 若無引號，過濾掉常見中文題幹與中文字，保留關鍵詞
        target_text = re.sub(r'請問.*?中文意思是什麼|的地熱/火熱詞彙是哪一個|聆聽音檔.*?|題目：|詞彙：|中文：.*', '', text)
    ...
    try:
        # 使用 gTTS 的印尼語發音 (lang='id')，因其發音極為接近南島語系與地熱名詞發音
...
def render_picture(line, prefix):
    ...
    try:
        ...
        try:
            ...
            img_path_jpg = f"assets/images/geothermal_{idx}.jpg"
            img_path_png = f"assets/images/geothermal_{idx}.png"
            ...
            if os.path.exists(img_path_jpg):
                ...
                st.info(f"🌋 火熱與地熱圖片佔位區：若要顯示圖片，請將圖片命名為 `geothermal_{idx}.jpg` 或 `.png`，並放置於 `assets/images/` 資料夾中。")
        except:
            pass

        st.markdown(f"🌋 **地熱情境：** {pic}")
        
        if hint:
            st.caption(f"中文提示：{hint}")
            
        st.text_area("請在此作答：", key=f"input_{prefix}", label_visibility="collapsed", placeholder="可以在此輸入您的地熱與火熱口說分析草稿...")
...
def render_dictation(line, prefix):
    ...
    try:
        ...
        if "中文：" in text:
            parts = text.split("中文：", 1)
            am = parts[0].replace("地熱專業語句：", "").strip()
            text = parts[1]
        ...
        st.text_area("請在此作答：", key=f"input_{prefix}", label_visibility="collapsed", placeholder="請在此輸入您聽寫的地熱相關專業句子...")
        ...
        with col_q:
            if st.toggle("👁️ 顯示聽寫原文", key=f"t_show_dict_{prefix}"):
                ...
                st.markdown("**[原文隱藏中，請點擊右側按鈕進行地熱語句聽寫測試]**")
...
def render_section(section_name, db):
    ...
    if not questions:
        st.warning(f"⚠️ 系統抓不到【{section_name}】的地熱專題資料。")
        return
...
# ==========================================
def main():
    st.set_page_config(page_title="火熱與地熱能源認證", page_icon="🌋", layout="centered", initial_sidebar_state="collapsed")

    # 極簡地熱熱能風 (Geothermal & Thermal Style) CSS
    st.markdown("""
    <style>
    .quiz-card {
    ...
    hr { border-top: 1px solid #F3D1C9; }
    </style>
    """, unsafe_allow_html=True)

    st.title("🌋 火熱與地熱能源專業認證")
    st.caption("[請選擇練習平台]")

    main_options = ["📋 認證指南說明", "🎧 地熱聽力", "🗣️ 地熱口說", "📖 地熱閱讀", "✍️ 地熱寫作"]
    ...
    if current_tab == "📋 認證指南說明":
        st.subheader("📋 [火熱與地熱技術認證說明](https://www.eere.energy.gov/geothermal/)")
        st.divider()
        st.info("請透過上方導覽列選擇您要進行的熱能測驗項目。系統將自動從資料庫載入火熱與地熱專題題庫，並附帶動態專業詞彙發音按鈕。")

    elif current_tab == "🎧 地熱聽力":
        st.subheader("🎧 地熱聽力測驗 (Geothermal Listening)")
        st.divider()
    ...
    elif current_tab == "🗣️ 地熱口說":
        st.subheader("🗣️ 地熱口說測驗 (Thermal Speaking)")
        st.divider()
    ...
    elif current_tab == "📖 地熱閱讀":
        st.subheader("📖 地熱閱讀測驗 (Geothermal Reading)")
        st.divider()
    ...
    elif current_tab == "✍️ 地熱寫作":
        st.subheader("✍️ 地熱寫作測驗 (Thermal Writing)")
        st.divider()
    ...
    st.write("---")
    st.caption(f"© 2026 火熱與地熱能源認證 App 三一開發團隊 ｜ 系統版本： **{APP_VERSION}** ")
